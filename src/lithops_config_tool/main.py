...
from config.config_tool.util_func import init_constants, update_config_file, get_option_from_list, get_resource_groups,\
...
CONFIGURABLE_STORAGE = {'IBM COS': {'config_title': 'ibm_cos', 'config_func': config_cos}, 'Other...': ''}
...
def config_generator(output_path, iam_api_key):
    ...
    selected_storage_backends = get_option_from_list('Please choose one OR MORE storage backend you would like to '
                                                     'configure:',
                                                     CONFIGURABLE_STORAGE.keys(), multiple_choice=True)['answer']
    selected_compute_backends = get_option_from_list('Please choose one OR MORE compute backend you would like to '
                                                     'configure:',
                                                     CONFIGURABLE_COMPUTE.keys(), multiple_choice=True)['answer']
    # execute selected backend configuring functions
    ...
    # user will choose default backends
