...
def convert_to_lithops(data):
    result = {'ibm_vpc': {}, 'ibm': {'iam_api_key': data['iam_api_key']}}
    ...
    result['ibm_vpc']['ssh_key_filename'] = data['ssh_key_path']

    result['ibm_vpc']['ssh_user'] = 'root'
    result['ibm_vpc']['delete_on_dismantle'] = True
    result['ibm_vpc']['volume_tier_name'] = data['volume_profile_name']
    result['ibm_vpc']['profile_name'] = data['instance_profile_name']
...
def print_to_file(format, output_file, result, input_file):
    ...
    if format == 'ray':
        ...
        defaults_config_path = input_file or path.abspath(path.join(__file__ ,"../../etc/lithopscloud/ray-defaults.yaml"))
        template_config, _, node_template, _ = get_ray_template_config(input_file)
        ...
        defaults_config_path = input_file or path.abspath(path.join(__file__ ,"../../etc/lithopscloud/lithops-defaults.yaml"))

        with open(defaults_config_path) as f: #TODO: find path
            config = yaml.safe_load(f)
            config['ibm_vpc'].update(result['ibm_vpc'])
            config['ibm'].update(result['ibm'])
...
def get_ray_template_config(input_file):
    template_config = {}
    provider_template = {}
    node_template = {}
    node_config = {}
...
@click.option('--input-file', '-i', help=f'Template for new configuration, default: {path.abspath(path.join(__file__ ,"../../etc/lithopscloud/defaults.yaml"))}')
...
def builder(output_file, input_file, iam_api_key, region, vpc_id, sec_group_id, ssh_key_id, image_id, instance_profile_name, volume_profile_name, head_ip, format):
    ...
    provider_template = {} # TODO: add support for lithops
    node_config = {}
    template_config = {}
    node_template = {}
    if format == 'ray':
        template_config, provider_template, node_template, node_config = get_ray_template_config(input_file)
